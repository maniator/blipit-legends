...
async function expandAllSections(page: Page): Promise<void> {
  ...
  await expect(page.locator('[data-testid="help-page"] details[open]')).toHaveCount(
    HELP_SECTION_COUNT,
  );
}
...
test.describe("Help page — all sections present", () => {
  ...
  test("all accordion sections are present", async ({ page }) => {
    const sections = page.locator('[data-testid="help-page"] details');
    await expect(sections).toHaveCount(HELP_SECTION_COUNT);
  });
