    ...
    getDb()
      .then(async (db) => {
        ...
        if (docs.length > 0) {
          ...
          const totalGameDays = getTotalGameDays(s.preset, s.seasonLength);
          const displayDay = Math.min(s.currentGameDay + 1, totalGameDays);
          setActiveSeasonLabel(`${s.name} · day ${displayDay} / ${totalGameDays}`);
        } else {
