  ...
  return {
    ...
    async createCustomTeam(input: CreateCustomTeamInput, meta?: { id?: string }): Promise<string> {
      ...
      if (duplicateDoc) {
        ...
        // Allow an autogenerated team to silently replace an existing autogenerated
        // team with the same name — this covers the common case of starting a
        // second season with fresh autogenerated teams that share names (e.g.
        // "Rockets") with the previous season's now-unused autogen roster.
        // We only do this when BOTH teams carry the autogen marker AND the
        // existing team is not locked by an active season.
