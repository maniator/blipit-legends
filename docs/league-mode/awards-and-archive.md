...
| **Reliever of the Year** | Relief pitchers: ≥ 25 appearances **and** < 2.0 IP per appearance (to exclude spot-starters). Pitchers who started ≥ 5 games are starter-track and ineligible.                                                                                                       | `0.45 × ERA-proxy (inverted) + 0.25 × appearances + 0.20 × inherited-runners strand-rate (if tracked; otherwise omit term and re-weight ERA/appearances to 0.55/0.30) + 0.10 × team W-L bonus`                                                                                                                                                                                                                                                                     |
...
- When triggered, on "Start New Season":
  ...
  - Compute `checksum = fnv1a(raw JSON before gzip)` (hex string, same `fnv1a` helper from `@storage/hash`).
