...
- **Unit:** `seasons.awards[]` byte-stability — write the same award set in two different insertion orders to two `seasons` docs (otherwise identical), then export both bundles → assert their `sig` values are byte-equal. Regression-tests the awards-array sort contract documented in `data-model.md` §`seasons.awards[]`.
