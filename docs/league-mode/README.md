...
- **Seed contract.** Per-game RNG seeds are derived deterministically: `fnv1a(\`${leagueSeasonId}:${scheduledGameId}\`) → uint32 → base-36`. Never pass raw colon-joined strings into `reinitSeed`. The cached value is round-tripped via `parseInt(s, 36) >>> 0`before reaching`mulberry32`. Helper: `deriveScheduledGameSeed()`in`src/features/league/utils/`.
