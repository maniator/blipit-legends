...
| `PlayoffBracketTree`                         | v3    | Tablet/desktop bracket — horizontal tree with connector lines (new visual token; pre-clear with Sally (bmad-agent-ux-designer) per [`docs/style-guide.md`](../style-guide.md)). Selected via `${mq.notMobile}`.              |
