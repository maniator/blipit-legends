...
All path-segment IDs (`seasonId`, `seasonTeamId`, `seriesId`, `seasonGameId`) are nanoid-style alphanumerics generated by `@storage/generateId` helpers. URL-encoding on construction is mandatory for forward compatibility but is a no-op for the current ID scheme — no path-segment ID contains a colon, slash, or other reserved character. (Composite-key style IDs like `${seasonId}:${customTeamId}` are deliberately confined to non-routed primary keys; anything that appears in a route uses a plain nanoid. See [`data-model.md`](data-model.md).)
