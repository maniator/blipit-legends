...
League IDs are generated as `lg_<fnv1a>` — they contain only lowercase hex characters and an underscore, so `encodeURIComponent` is a no-op. However, for consistency with the existing pattern for team IDs, always `encodeURIComponent` any ID used as a URL path segment when building navigation URLs. This protects against any future ID format changes.
