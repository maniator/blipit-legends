...
League IDs are generated by `generateLeagueId()` from `@storage/generateId` (nanoid-based, URL-safe by construction). As a forward-looking safety measure, always `encodeURIComponent` any ID used as a URL path segment when building navigation URLs. This protects against any future ID format changes.
