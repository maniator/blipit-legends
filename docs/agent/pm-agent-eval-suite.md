...
- Citation: `src/features/gameplay/context/hitBall.ts:204-210`; `src/features/gameplay/context/playerOut.ts:50-58`; `src/shared/utils/stats/computeBattingStatsFromLogs.ts:83-86` (the `entry.isSacFly` branch increments `sacFlies` instead of `atBats`).
