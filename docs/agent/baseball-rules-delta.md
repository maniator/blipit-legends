...
| IBB availability                        | Manager may issue IBB at any time                                      | Only offered in Manager Mode when: inning ≥ 7, 2 outs, score diff ≤ 2, 1st base empty, runner on 2nd or 3rd, AND defense is not trailing by 2+ (issuing a free baserunner while already trailing stacks bad outcomes) | ⚠️ Delta (constrained availability) | `reducer.ts` `detectDecision`                                             |
...
| Steal availability        | Manager's discretion                                   | Manager Mode only; offered when expected success > `stealMinOfferPct` (default 72), <2 outs, destination base empty, AND `decisionValues.stealEnabled` is true (master switch — when false, no steal is ever offered or attempted by either side) | ⚠️ Delta (constrained)      | `src/features/gameplay/context/reducer.ts` `detectDecision`, `src/features/gameplay/context/managerDecisionValues.ts` |
...
| Bunt availability           | Manager may signal bunt at any time                         | Offered in Manager Mode when: inning ≥ 6, <2 outs, runner on 1st or 2nd (1 out requires runner on 2nd), strikes < 2, balls ≤ 1, AND batting team is tied or trailing by ≤ 2 runs (bunting while leading reduces win expectancy) | ⚠️ Delta (constrained availability)    | `reducer.ts` `detectDecision` |
