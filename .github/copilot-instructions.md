...
- **Playwright MCP localhost bootstrap** — in agent sessions, start vite preview with `--host 0.0.0.0` (`nohup npx vite preview --port 5173 --host 0.0.0.0 >> vite-preview.log 2>&1 & disown`) then navigate MCP Chrome to `http://127.0.0.1:5173`. The Playwright `webServer` approach also works but is not the only option. Prerequisite: `--no-sandbox` must be in the `mcp-server-playwright` MCP server args (Settings → Copilot → MCP servers → playwright-mcp); without it Chrome fails to start. See `docs/e2e-testing.md` § "Starting the preview server for MCP browser automation" for full details.
